...
primary spermatocyte also includes a couple hermaphrodites - let's chuck em out
...
And now with PIC, figure 1D. This is plotting mean vs cv of the traits, by PIC
...
plotpic(oocytesizearea1,tr)
```

Let's try some PIC of spermsize vs oocyte size!

```{r}
plotpic2<-function(df1,df2,tree,title=""){
...
plotpic2(spermsize1,oocytesizearea1,tr)
plotpic2(primaryspermatocyte1,oocytesizearea1,tr)
plotpic2(oocytesizearea1,oocytesize1,tr)
```

pgls - first with lambda, then with OU model.
...
mod3ou <-gls(meansperm~meanprimsp,correlation=corMartins(1,phy=tr),data=fullpglsframe)
```

pgls using the ou method
