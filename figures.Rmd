...
```{r, echo=FALSE}
totaldata<-as.data.frame(pic(spermsize1$means,tr))
names(totaldata)<-c("spermsize")

totaldata$oocytesizearea<-pic(oocytesizearea1$means,tr)
totaldata$oocytesize<-pic(oocytesize1$means,tr)
totaldata$malebodyarea<-pic(bodysizearea1[bodysizearea1$Sex=="male",]$means,tr)
totaldata$femalebodyarea<-pic(bodysizearea1[bodysizearea1$Sex=="female",]$means,tr)
totaldata$malebodylength<-pic(bodysizelength1[bodysizelength1$Sex=="male",]$means,tr)
totaldata$femalebodylength<-pic(bodysizelength1[bodysizelength1$Sex=="female",]$means,tr)
totaldata$malebodywidth<-pic(bodysizewidth1[bodysizewidth1$Sex=="male",]$means,tr)
totaldata$femalebodywidth<-pic(bodysizewidth1[bodysizewidth1$Sex=="female",]$means,tr)

data2<-merge(totaldata,pic(eggsize1$means,drop.tip(tr,tr$tip.label[!(tr$tip.label %in% eggsize1$Species)])),by=0,all.x=T)
data3<-merge(totaldata,pic(primaryspermatocyte1$means,drop.tip(tr,tr$tip.label[!(tr$tip.label %in% primaryspermatocyte1$Species)])),by=0,all.x=T)
